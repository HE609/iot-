...
\begin{table}[H]
\centering\caption{OFDM雷达仿真参数（5G NR n79，跨时隙慢时间多普勒处理）}\label{tab:param}
\begin{tabular}{cccc}
\toprule
参数 & 符号 & 数值 & 说明 \\
\midrule
...
每时隙符号数 & $N_{\mathrm{sym}}$ & 14 & 时隙周期$T_{\mathrm{slot}}\approx0.5$\,ms \\
CPI时隙数 & $N_{\mathrm{CPI}}$ & 14 & 相参积累时长$\approx7$\,ms \\
距离分辨率 & $\Delta r$ & 1.50\,m & $c/(2B)$ \\
最大探测距离 & $R_{\max}$ & 5000\,m & $c/(2\Delta f)$ \\
速度分辨率 & $\Delta v$ & 4.4\,m/s & $\lambda/(2N_{\mathrm{CPI}}T_{\mathrm{slot}})$ \\
最大不模糊速度 & $v_{\max}$ & 30.6\,m/s & $\lambda/(4T_{\mathrm{slot}})$ \\
\bottomrule
\end{tabular}
\end{table}
...
上述仿真采用\textbf{跨时隙慢时间处理}：在每个时隙内对$N_{\mathrm{sym}}=14$个OFDM符号做相参积累得到一路距离剖面，再对$N_{\mathrm{CPI}}=14$个时隙的距离剖面序列做慢时间FFT，从而获得速度估计。需要注意以下两个性能边界问题：

\textbf{（1）速度分辨率与多普勒混叠。}在$N_{\mathrm{CPI}}=14$时隙、积累时长约7\,ms的条件下，速度分辨率为
\[
  \Delta v = \frac{\lambda}{2N_{\mathrm{CPI}}T_{\mathrm{slot}}} \approx 4.4\,\text{m/s}
\]
对于悬停或低速多旋翼无人机（$<$2\,m/s）的分辨能力仍有限，可通过增大$N_{\mathrm{CPI}}$进一步改善。最大不模糊速度$v_{\max}=\lambda/(4T_{\mathrm{slot}})\approx30.6$\,m/s，仿真中eVTOL的速度$-25$\,m/s已接近此边界，若目标速度超过$v_{\max}$将发生多普勒折叠。实际系统中可结合\textbf{交错PRF}技术（改变时隙发射间隔）扩展不模糊速度范围。
